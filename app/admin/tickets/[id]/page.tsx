...
function TicketCard({ ticket, onStatus, disabled }: { ticket: Ticket; onStatus: (status: TicketStatus) => void; disabled: boolean }) {
  return (
    <div className="mb-8 rounded-2xl border border-border bg-card p-6">
      ...
      <div className="mt-4 grid gap-2 text-sm text-muted-foreground sm:grid-cols-2"><p><span className="font-medium text-foreground">Tel.:</span> {ticket.phone}</p><p><span className="font-medium text-foreground">Ciudad:</span> {ticket.city}</p><p><span className="font-medium text-foreground">Ubicación:</span> {ticket.location}</p><p><span className="font-medium text-foreground">Creado:</span> {new Date(ticket.createdAt).toLocaleDateString("es-US")}</p></div>
