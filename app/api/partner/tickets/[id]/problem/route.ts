...
import { validatePartnerId, validateProblemInput } from "@/lib/validation";
...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    const problem = validateProblemInput(body);
    const { error } = await getSupabaseAdmin().rpc("partner_report_problem", {
      p_ticket_id: id,
      p_appointment_id: appointmentId,
      p_partner_user_id: partnerUser.id,
      p_category: problem.category,
      p_notes: problem.notes,
    });
